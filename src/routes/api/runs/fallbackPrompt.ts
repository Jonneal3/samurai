interface CombinedData9 {
    ...
    task: { fallback_prompt?: string }[]; // Adjust the type according to your data structure
...
const fetchActionPrompt = async ({ knowledge, task, memory }: CombinedData9): Promise<any[]> => {
    ...
    if (Array.isArray(task) && task.length > 0 && task[0].fallback_prompt !== undefined) {
        systemPrompts.push({
            role: 'system',
            content: task[0].fallback_prompt,
        });
    } else {
